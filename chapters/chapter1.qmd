...
classification. The flat model ignores the hierarchy in the data, and is used as a benchmark for the other models. The three hierarchical approaches are:
...
![Stacked Model Architecture]('images/stacked_model_hier.png')
...
higher the penalty. These penalty values are set in a penalty matrix. During training,
...
learn to favor classes that are closer in distance to the true class to avoid high penalties. We find that using only this custom loss leads to a decrease in performance, which is why we use the cross entropy
in combination with our hierarchical loss function. The final loss
