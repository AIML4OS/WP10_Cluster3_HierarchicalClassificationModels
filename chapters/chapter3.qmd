...
accuracy takes the hierarchy into account, by counting how the number
of levels are predicted correctly up to the first incorrect level. If
...
assigned values for all output codes.
...
However, not only is this increase in performance minimal.
The performance would have been poorer had the loss function been based
solely on the hierarchically informed distance penalty matrix, not
combined with cross-entropy loss (ratio 7:3). The stacked and
...
As compared with a flat model, one of the hierarchical models increased performance slightly.
This was the version that is considered hierharchical because its loss function takes input in the form of penalties from a matrix describing the similarity of the classes based on the assumed hierarchy.
Even that loss function, however, was in part (30%) also based on "flat" cross-entropy loss.
The stacked model (with one classifier per level, LCL) and the multiple outputs model performed worse.
