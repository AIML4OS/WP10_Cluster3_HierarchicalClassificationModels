...
# evaluation matrics={accuracy, top-3 accuracy, top-5 accuracy, top-10 accuracy, hierarchical accuracy}
...
use_hier_loss <- FALSE #ob hierarchische loss funktion verwendet werden soll
...
# ------------------------------------------------------------------------------------------------------
# tokenization of data with 3-gram
...
# one hot encode 3-gram and 5-gram
x <- setupOneHot(x_1)
x2 <- setupOneHot(x_2)
...
if(use_hier_loss){
  ...
  nace_codes <- levels(y_lookup$V1) #Achtung müssen ordered sein (aufsteigend)
...
result_flat <- data.table(metric=c("accuarcy","top3_accuracy","top5_accuracy","top10_accuracy","hier_accuracy"),
                          value=c(acc,top3,top5,top10,hier_acc),
                          type="flat")
openxlsx::write.xlsx(result_flat,
                     paste0("data/results_flat_class",Sys.Date(),".xlsx"))
